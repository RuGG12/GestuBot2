...
class GestureBotInference:
    ...
    def __init__(self, confidence_threshold: float = 0.7):
        ...
        self.conf_filter = ConfidenceFilter(threshold=confidence_threshold)
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[int, float, Optional[np.ndarray], dict, float]:
        """
        Process a single frame through the full pipeline.

        Returns:
            Tuple of (prediction, latency_ms, contour, stage_times, confidence)
        """
        stage_times = {}
        ...
        # 3b. Confidence filter (reject uncertain predictions)
        t0 = time.perf_counter()
        raw_prediction, confidence = self.conf_filter.filter(raw_prediction, confidence)
        if features is not None:
            self.conf_filter.log_uncertain(raw_prediction, confidence, features)
        stage_times['conf_filter'] = (time.perf_counter() - t0) * 1000
        ...
        return filtered_prediction, latency_ms, contour, stage_times, confidence
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                # Process frame
                prediction, latency_ms, contour, stage_times, confidence = self.process_frame(frame)
                ...
                cv2.putText(
                    display, f"Confidence: {confidence:.2f}", (FRAME_WIDTH - 250, 55),
            ...
            # Print confidence filter stats
            conf_stats = self.conf_filter.summary()
            if conf_stats['total'] > 0:
                print(f"\n  Confidence filter (threshold={conf_stats['threshold']:.2f}):")
                print(f"    {conf_stats['rejected']}/{conf_stats['total']} predictions rejected")
                print(f"    Rejection rate: {conf_stats['rejection_rate']:.1%}")

            self.conf_filter.close()
...
def main():
    ...
    parser.add_argument(
        ...
        help='Min confidence to accept a prediction (default: 0.7)'
    )
